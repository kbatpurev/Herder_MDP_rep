...
results <- simulate_adaptive(step=1400)
```
...
ggplot(degradation_summary, aes(x = step, y = pct_degraded)) +
  ...
  labs(title = "Degradation Over Time - Action based PES", y = "% Degraded", x = "Step")
```
...
ggplot(policy_map, aes(x = x, y = y, fill = action)) +
  ...
  labs(title = "Dominant Stocking Rate by Agents - Action based PES", x = "land_grid X", y = "land_grid Y")
```
...
Ecosystem degradation rate is similar to Action based PES. Landscape reaches complete degradation in ~260 years. 
...
ggplot(degradation_summary, aes(x = step, y = pct_degraded)) +
  geom_line(size = 1.2, color = "darkred") +
  labs(title = "Degradation Over Time - Outcome based PES", y = "% Degraded", x = "Step")
```
...
ggplot(policy_map, aes(x = x, y = y, fill = action)) +
  ...
  facet_wrap(~agent) +
  scale_fill_brewer(palette = "Set2") +
  labs(title = "Dominant Stocking Rate by Agents - Outcome based PES", x = "land_grid X", y = "land_grid Y")
```
...
```{r action selection nopes, echo=FALSE, include=FALSE}
select_action <- function(agent) {
  candidates <- expand.grid(x = 0:(land_grid_x - 1), y = 0:(land_grid_y - 1), action = actions)
  candidates$reward <- apply(candidates, 1, function(row) {
    key <- paste(row[["x"]], row[["y"]], row[["action"]], sep = "_")
...
Ecosystem degradation rate seems slightly faster than action and outcome based PES. 
...
ggplot(degradation_summary, aes(x = step, y = pct_degraded)) +
  geom_line(size = 1.2, color = "darkred") +
  labs(title = "Degradation Over Time - No PES", y = "% Degraded", x = "Step")
```
...
ggplot(policy_map, aes(x = x, y = y, fill = action)) +
  ...
  facet_wrap(~agent) +
  scale_fill_brewer(palette = "Set2") +
  labs(title = "Dominant Stocking Rate by Agents - No PES", x = "land_grid X", y = "land_grid Y")
```