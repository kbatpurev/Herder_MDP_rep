...
The dominant stocking rate seem slightly different between the two agents. This is totally random, but partly could be because the There is still a small chance that these will and  is **below 400** goats (>13 cells out of 25) for agent 1 
...
ggplot(policy_map_weather, aes(x = x, y = y, fill = action)) +
  ...
  labs(
    title = "Dominant Stocking Rate by Agents under Different Weather Regimes",
    x = "Grid X", y = "Grid Y"
  ) +
  theme_minimal(base_size = 12) +
  theme(
...
When PES scheme incentive is outcome (rangeland condition) based, agents tend to employ a higher stocking rate compared to action based PES (herd size). Dominant stocking rate is **above 400** for majority of the cells (>14 out of 25).  
